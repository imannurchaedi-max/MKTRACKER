...
function getAbsenReport(nik, deptFilter, periodType, periodValue, page, pageSize) {
  try {
    ...
    const meta = buildPaginationMeta_(fullReport.rows.length, reportPage, reportPageSize);
    const visibleRows = fullReport.rows.slice(meta.startIndex, meta.endIndex);
    const result = {
      ok: true,
      period: fullReport.period,
      total: fullReport.total,
      complete: fullReport.complete,
      active: fullReport.active,
      data: visibleRows,
      ...
      startRow: fullReport.total === 0 ? 0 : meta.startIndex + 1,
...
function getAreaActivityReport(nik, deptFilter, periodType, periodValue, page, pageSize, recapPage, recapPageSize) {
  try {
    const targetNik = asText(nik).trim();
    const filterDpt = asText(deptFilter).trim();
    // Boleh kosong jika admin/security (tampil semua data periode)
    ...
    const rowMeta = buildPaginationMeta_(fullReport.rows.length, reportPage, reportPageSize);
    const visibleRows = fullReport.rows.slice(rowMeta.startIndex, rowMeta.endIndex);
    const recapMeta = buildPaginationMeta_(fullReport.recap.length, summaryPage, summaryPageSize);
    const visibleRekap = fullReport.recap.slice(recapMeta.startIndex, recapMeta.endIndex);
    const result = {
      ok: true,
      period: fullReport.period,
      total: fullReport.total,
      ...
      recapTotal: fullReport.recap.length,
      ...
      recapStartRow: fullReport.recap.length === 0 ? 0 : recapMeta.startIndex + 1,
