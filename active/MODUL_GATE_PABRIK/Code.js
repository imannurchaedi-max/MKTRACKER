...
function updateRecapAbsen(tanggal, nik, nama, dept, jabatan, jamMasuk, jamKeluar, noKartuMK, noLoker) {
  ...
  const recapContext = resolveRecapShiftContext(tanggal, nik, jamMasuk || jamKeluar, eventType);
  const recapDate = recapContext.tanggal || asText(tanggal);
  const key = makeRecapKey(recapDate, nik);
  let targetRow = 0;
  ...
  if (lastRow > 1) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (makeRecapKey(data[i][0], data[i][1]) === key) {
        targetRow = i + 2;
        existingJamMasuk = normalizeTimeValue(data[i][5]);
        existingJamKeluar = normalizeTimeValue(data[i][6]);
  ...
  const jamMasukNormalized = normalizeTimeValue(jamMasuk);
  const jamKeluarNormalized = normalizeTimeValue(jamKeluar);
  const finalJamMasuk = jamMasukNormalized
    ? (existingJamMasuk && compareTimeValues(existingJamMasuk, jamMasukNormalized) < 0 ? existingJamMasuk : jamMasukNormalized)
    : existingJamMasuk;
  const finalJamKeluar = jamKeluarNormalized
    ? (existingJamKeluar && compareTimeValues(existingJamKeluar, jamKeluarNormalized) > 0 ? existingJamKeluar : jamKeluarNormalized)
    : existingJamKeluar;
  const row = [
    asText(recapDate),
    asText(nik),
  ...
  sheet.getRange(targetRow, 6, 1, 2).setNumberFormat('@');
}
...
function rebuildRecapAbsenInOutMK() {
  return withDocumentLock(function() {
    try {
      ...
      const recapLastRow = recapSheet.getLastRow();
      if (recapLastRow > 1) {
        recapSheet.getRange(2, 1, recapLastRow - 1, recapWidth).clearContent();
      }
      ...
      function replayLog(sheetName, eventType) {
        const sheet = getSheet(sheetName);
        const data = sheet.getDataRange().getValues();
        let applied = 0;
        for (let i = 1; i < data.length; i++) {
          const noKartu = normalizeCard(data[i][0]);
          const nik = asText(data[i][1]).trim();
          if (!nik) continue;

          const master = getKaryawanByNIK(nik) || {};
          const nama = asText(data[i][2]) || asText(master.nama);
          const dept = asText(master.dept);
          const jabatan = asText(master.jabatan);
          const tanggal = normalizeSheetDateValue(data[i][3]);
          const jam = normalizeTimeValue(data[i][4]);
          const loker = asText(data[i][6]);

          if (!tanggal || !jam) continue;

          if (eventType === 'masuk') {
            updateRecapAbsen(tanggal, nik, nama, dept, jabatan, jam, '', noKartu, loker);
          } else {
            updateRecapAbsen(tanggal, nik, nama, dept, jabatan, '', jam, noKartu, loker);
          }
          applied++;
      ...
      const masukCount = replayLog(SHEET_MASUK_PABRIK, 'masuk');
      const keluarCount = replayLog(SHEET_KELUAR_PABRIK, 'keluar');
      const finalLastRow = recapSheet.getLastRow();
      if (finalLastRow > 1) {
        recapSheet.getRange(2, 6, finalLastRow - 1, 2).setNumberFormat('@');
      }

      const msg = 'Rekap ABSEN IN OUT MK berhasil digenerate ulang. Masuk: ' + masukCount + ', Keluar: ' + keluarCount + ', Total recap: ' + Math.max(0, finalLastRow - 1) + '.';
      try {
        SpreadsheetApp.getUi().alert(msg);
      } catch (_) {}
      return { ok: true, msg: msg, masukCount: masukCount, keluarCount: keluarCount, recapRows: Math.max(0, finalLastRow - 1) };
    } catch(e) {
...
function getBindingStatus(noKartuMK) {
  try {
    ...
    if (kar && !isExternalKaryawan(kar)) {
      const now = nowWIB();
      const recapContext = resolveRecapShiftContext(formatDate(now), kar.nik, formatTime(now), 'keluar');
      const todayStatus = getFactoryRecapStatus(kar.nik, recapContext.tanggal || formatDate(now));
  ...
  return withDocumentLock(function() {
    try {
      ...
      if (kar) {
        ...
        const tanggal = formatDate(now);
        const jam = formatTime(now);
        const recapContext = resolveRecapShiftContext(tanggal, kar.nik, jam, 'keluar');
        const factoryStatus = getFactoryRecapStatus(kar.nik, recapContext.tanggal || tanggal);
        if (factoryStatus !== 'DI DALAM') {
      ...
      const waktu = formatDateTime(now);
      const tanggal = formatDate(now);
      const recapContext = resolveRecapShiftContext(tanggal, binding.nik, formatTime(now), 'keluar');
      const factoryStatus = getFactoryRecapStatus(binding.nik, recapContext.tanggal || tanggal);
      if (factoryStatus === 'SELESAI') {
