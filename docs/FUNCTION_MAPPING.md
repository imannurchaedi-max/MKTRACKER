...
**Acuan**: `reports/function_inventory.md` (202 GAS functions, 354 frontend functions) · Audit terakhir: 2026-06-23
...
| `safeUpdateRecapAbsen(bindCtx)` | 47 | Wrapper aman untuk update recap |
| `getBindingStatus(noKartuMK)` | 56 | Cek status binding kartu (FREE/BOUND) |
| `bindKartu(noKartuMK, nik, loker)` | 98 | Proses masuk pabrik + binding kartu |
| `releaseKartu(noKartuMK, loker, mode)` | 169 | Proses keluar pabrik + release kartu |
...
| `detectShift(date)` | 290 | Deteksi shift berdasarkan waktu (1/2/3) |
| `timeStrToMinutes(ts)` | ~300 | Konversi HH:mm ke menit dari 00:00 |
| `withDocumentLock(fn)` | 303 | Document lock wrapper untuk write path |
